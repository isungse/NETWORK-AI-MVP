...
물리 포트 타입, 슬롯, 모듈 모델, 포트 수, 시리얼은 명시적인 장비 출력이나 동등한 운영자 제공 근거가 있을 때만 전면 패널에 사용한다. 근거가 없는 장비에는 아래 정보를 유추해 적용하지 않고 일반 logical fallback을 유지한다.

현재 `cisco-backbone`의 `WS-C4503-E`에는 운영자가 제공한 `show module` 결과를 확인된 참조 메타데이터로 사용한다.

| Slot | Ports | Card Type | Model | Serial |
| --- | ---: | --- | --- | --- |
| 1 | 4 | Sup 7-E 10GE (SFP+), 1000BaseX (SFP) | WS-X45-SUP7-E | CAT1834L1W7 |
| 2 | 24 | 1000BaseX SFP | WS-X4724-SFP-E | CAT1832L3HR |
| 3 | 48 | 10/100/1000BaseT EEE (RJ45) | WS-X4748-RJ45-E | CAT1648L0AN |

적용 시 다음 원칙을 지킨다.

- `Fa1`은 별도 관리 인터페이스로 취급하며 위 모듈 포트 수에 포함하지 않는다.
- 모듈 정체성과 시리얼은 확인된 참조 정보이고, 각 포트 상태는 최신 저장 관측이라는 점을 시각적으로 구분한다.
- 확인되지 않은 빈 슬롯, 모듈, 포트 타입은 임의로 생성하거나 표시하지 않는다.
- 다른 장비나 모델에는 해당 섀시 구성을 재사용하지 않는다.

### 인터페이스 MAC 표시와 진단 규칙

`endpoint_macs`는 해당 인터페이스에서 포워딩 테이블을 통해 학습된 MAC 근거다. 직접 연결된 단일 단말 목록이라고 단정하지 않는다.

- 학습된 MAC이 있으면 개수와 전체 목록을 표시한다. 첫 항목만 남기거나 임의로 생략하지 않는다.
- `mac_table_collected`가 거짓이면 `MAC table not collected`로 표시하여, 수집했지만 결과가 없는 상태와 구분한다.
- MAC 테이블이 수집되었지만 연결 포트에서 MAC이 없으면 `Connected, no MAC learned`로 표시한다. 무트래픽, 에이징, 절전 상태 등 가능한 원인이 있으므로 장애로 단정하지 않는다.
- down 또는 disabled 포트에 MAC이 없으면 해당 운영 상태와 함께 `No MAC learned`로 표시한다.
- MAC 한 개와 여러 개 학습 상태를 구분한다. 여러 MAC은 업링크, 트렁크, AP, 브리지, 전화기와 PC 조합, 가상화 호스트 등에서 발생할 수 있으므로 개수만으로 업링크라고 확정하지 않는다.
- 모든 결과는 저장된 관측 시점의 근거이며 실시간 포워딩 테이블이라고 표현하지 않는다.
